...
app.post('/api/add.php', function (req, res) {
    ...
    db.execute("insert into `record` values(?,?,?,'',?,?,?,0,0,0)", [uuid, req.body.code, req.body.input, req.body.time, req.body.memory, req.body.lang], function (err, rows) {
        if (!(0, _paramsChecker.handler)(err, res)) return;
        res.end(JSON.stringify({ err: null, uuid: uuid }));
    });
});

app.get('/api/get.php/:uuid', function (req, res) {
    db.execute("select `uuid`,`output`,`judged` from `record` where `uuid`=?", [req.params.uuid], function (err, rows, cols) {
        if (!(0, _paramsChecker.handler)(err, res)) return;
